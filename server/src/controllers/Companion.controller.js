  ...
  static async addCompanion(req, res) {
    ...
    try {
      ...
      if (road.userId !== user.id) {
        return res
          .status(403)
          .json({ message: 'У вас нет прав на добавление компаньонов' });
      }
      ...
      if (companionByEmail.id === user.id) {
        return res
          .status(400)
          .json({ message: 'Вы не можете добавить себя в компаньоны' });
      }

      const companionInRoad = await CompanionService.getByRoadIdAndUserId(
        Number(roadId),
        Number(companionByEmail.id),
      );
      if (companionInRoad) {
        return res.status(400).json({ message: 'Компаньон уже добавлен' });
      }

      const companion = await CompanionService.addCompanion(
        Number(roadId),
        Number(companionByEmail.id),
      );
  ...
  static async removeCompanion(req, res) {
    ...
    try {
      ...
      if (isOwner) {
        ...
        return res
          .status(200)
          .json(formatResponse(200, 'Компаньон удален владельцем маршрута'));
      }
      ...
      return res
        .status(403)
        .json(formatResponse(403, 'У вас нет прав на удаление компаньона'));
    } catch (error) {
