...
class GigaChatService {
  static async getRecommendations(city, type) {
    ...
    try {
      ...
      if (type === 'items') {
        content = `Перечисли необходимые вещи, связанные с поездкой в город ${city}.Не пиши ничего кроме названий вещей.Максимум 200 символов.Фразы должны быть законченными предложениями.Дописывай слова`;
      } else if (city) {
        content = `Перечисли основные достопримечательности в ${city} в виде: название места, адрес места, краткое описание. Максимум 200 символов.Фразы должны быть законченными предложениями.`;
      } else {
